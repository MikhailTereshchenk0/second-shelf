...
| `book-service` внутренние API | `/internal/books/{id}`, `/internal/books/owners/{ownerId}/available-public`, `/reserve`, `/available`, `/exchanged` | `exchange-service` и доверенные сервисы с `X-Internal-Token` | `INTERNAL_SERVICE` authority | Используются для проверки книг, показа владельцу доступных публичных книг заявителя и согласования состояний книг в обменах |
| `exchange-service` создание обмена | `POST /api/v1/exchanges` | Аутентифицированный пользователь | Bearer JWT + бизнес-проверки | Пользователь запрашивает публичную доступную книгу другого владельца без выбора своей книги взамен |
...
| `exchange-service` предложение владельца | `POST /api/v1/exchanges/{id}/offer`, `/decline` | Только владелец запрошенной книги | Проверка `ownerId` | Владелец может выбрать публичную доступную книгу заявителя или отклонить `PENDING` заявку |
| `exchange-service` финальное решение заявителя | `POST /api/v1/exchanges/{id}/accept`, `/decline-offer` | Только заявитель | Проверка `requesterId` | Заявитель принимает или отклоняет предложение владельца; книги резервируются только при финальном accept |
