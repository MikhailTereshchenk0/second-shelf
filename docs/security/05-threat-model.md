...
| Подбор паролей и credential stuffing | `auth-service`, `user-service` | bcrypt-хэширование, запрет логина отключенным пользователям | rate limiting, WAF, антибот-защита, мониторинг аномалий |
...
| Утечка ПДн через логи | все сервисы | в коде нет необходимости логировать пароль или токены; есть correlation id в `exchange-service` и `notification-service` | централизованная маскировка, правила запрета логирования секретов, контроль доступа к логам |
...
- `exchange-service` и `notification-service` публикуют `/actuator/metrics` без аутентификации, поэтому доступ к ним должен быть закрыт внешним периметром.
